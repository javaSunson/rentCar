...
public class ChangeCarView {
	...
	public static void main(String[] args) {
		try {
			ChangeCarView window = new ChangeCarView();
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void createContents() {
		...
		ui_car_isrent.setToolTipText("请认真输入");
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				int change = cars.changeCar(ui_car_brand.getText(), ui_car_model.getText(), 
						ui_car_num.getText(), ui_car_price.getText(), ui_day_money.getText(), ui_car_isrent.getText());
				if( change == 1) {
					ui_tips.setText("修改成功!");
				}else {
					ui_tips.setText("该品牌和型号不存在!");
				}
			}
		});
